...
import { GET_MY_BOOKINGS } from "../../graphql/booking.operations";
...
export const MyBookings = () => {
  const { data, loading, error } = useQuery<GetMyBookingsData>(GET_MY_BOOKINGS);
  ...
  return (
    <div className="flex flex-col gap-4">
      {bookings.map((booking) => (
        <div
          key={booking.id}
          className="w-full border border-[#87a700] rounded-2xl bg-white p-5 sm:p-8"
        >
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5 sm:mb-6 pb-4 border-b border-gray-200">
            ...
            <span className="self-start sm:self-auto px-3 py-1 rounded-full bg-[#87a700] text-[#fdffe9] text-xs font-medium font-[family-name:var(--font-text)]">
